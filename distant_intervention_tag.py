...
stopwords.extend(["either", "time", "patients"])
...
def _tag_vs(study):
    ...
    for iv in interventions:
        ...
        matched = longest_common_substrings(
                            abstract_tokens, 
                            iv_tokens)
        ...
        if max_len == 0:
            ...
            ### TODO actually 'tag' (i.e., put the index spans in!)
...
def ds_interventions_abstracts(bviewer, num_studies=None):
    tagged_pmids, tagged_abstracts, abstracts = [], [], []
    count = 0
    for study in bviewer:
        ...
        if intervention_text is None or abstract_text == "":
            ...
            if "vs" in intervention_text.split(" "):
                if not _is_drug_trial(study):
